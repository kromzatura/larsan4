...
describe("resolveHref", () => {
  ...
  it("resolves product pages (default locale en)", () => {
    expect(resolveHref(DOC_TYPES.PRODUCT, "widget")).toBe(
      "/en/products/widget"
    );
  ...
  it("resolves product categories (default locale en)", () => {
    expect(resolveHref(DOC_TYPES.PRODUCT_CATEGORY, "gadgets")).toBe(
      "/en/products/category/gadgets"
    );
...
describe("resolveDocHref", () => {
  ...
  it("handles string slug field (default locale en)", () => {
    expect(resolveDocHref({ _type: DOC_TYPES.POST, slug: "post-1" })).toBe(
      "/en/blog/post-1"
    );
  });
  ...
  it("handles slug object field (default locale en)", () => {
    expect(
      resolveDocHref({ _type: DOC_TYPES.PAGE, slug: { current: "features" } })
    ).toBe("/en/features");
  });
